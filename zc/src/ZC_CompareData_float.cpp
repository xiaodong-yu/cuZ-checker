...
size_t r5, size_t r4, size_t r3, size_t r2, size_t r1)
{
    ...
    duration += grid_sum(data1, numOfElem);
    duration += grid_sum(data2, numOfElem);
    duration += grid_min((float*)diff, numOfElem);
    duration += grid_max((float*)diff, numOfElem);
    duration += grid_sum((float*)diff, numOfElem);
    duration += grid_sum((float*)diff, numOfElem);
    duration += grid_min((float*)diff, numOfElem);
    duration += grid_max((float*)diff, numOfElem);
    duration += grid_sum((float*)diff, numOfElem);
    duration += grid_sum((float*)diff, numOfElem);
    cout << "Type one execution time:" << duration << "ms" << endl;
    ...
    float *autocor = autoCorr(ddata1, r3, r2, r1, avgDiff, order*2);
	...
	{
		int status = 0;
		compareResult->derivative1_sobolev = zc_calc_metric_der_order1_sobolev_p2_float(data1, data2, dim, r4, r3, r2, r1, &status);
		//printf("compareResult->derivative1_sobolev = %f\n", compareResult->derivative1_sobolev);
		if(status!=0)
